...
// ── PROVENANCE survives a rename of the label, because it is not the label ──
//
// `originKey` embeds the plan's label and `seenOrigins` uses the result to know a card is
// already on the canvas. That makes the label half of a PERSISTED key, not a caption — so
// renaming the generated plan's label from "Generated by CHART" to "Generated by NU Map"
// silently gave every student who had saved one a complete duplicate set of reservations the
// next time they generated. Measured before the fix: 3 cards became 6, with no note.
//
// The corpus fingerprint gate cannot catch this. It hashes the emitted document, and this is
// about state already sitting in the student's browser — which is exactly why it needs a test
// here rather than a number in a sweep.

import {
  GENERATED_PLAN_LABEL, LEGACY_GENERATED_PLAN_LABEL, GENERATED_ORIGIN_NS, isGeneratedPlanLabel,
} from "../../src/core/planLabels.js";

const PROV_SEMS = [
  { id: "f26", semTypeId: "fall", type: "fall", label: "Fall 2026", maxSlots: 5 },
  { id: "s27", semTypeId: "spring", type: "spring", label: "Spring 2027", maxSlots: 5 },
];
const PROV_PLAN = { years: [{ label: "Year 1", terms: [
  { term: "Fall", type: "fall", entries: [
    { text: "Khoury Approved Elective", sh: 4, options: [] },
    { text: "Mathematics Elective", sh: 4, options: [] },
  ] },
  { term: "Spring", type: "spring", entries: [{ text: "General Elective", sh: 4, options: [] }] },
] }] };
const applyAs = (label, prev) => applySamplePlan({ ...PROV_PLAN, label }, {
  semesters: PROV_SEMS, courseMap: {},
  placements: prev?.placements ?? {}, reservations: prev?.reservations ?? {},
});

test("provenance › regenerating after the label was renamed does NOT duplicate cards", () => {
  const first = applyAs(LEGACY_GENERATED_PLAN_LABEL);
  const n = Object.keys(first.reservations).length;
  assert.equal(n, 3, "fixture should reserve three cells");
  const again = applyAs(GENERATED_PLAN_LABEL, first);
  assert.equal(Object.keys(again.reservations).length, n,
    "a student who saved under the old label and regenerated got every card twice");
  assert.ok((again.notes ?? []).every(x => x.kind === "already-reserved"),
    "the de-dupe should recognise them, not silently re-add");
});

test("provenance › the origin namespace is INDEPENDENT of the display label", () => {
  // The property that makes the next rename free. If these are ever equal by construction
  // rather than by history, the bug is back the moment someone edits the caption.
  assert.notEqual(GENERATED_PLAN_LABEL, GENERATED_ORIGIN_NS,
    "the caption has changed and the persisted key has not — that is the whole point");
  assert.equal(GENERATED_ORIGIN_NS, LEGACY_GENERATED_PLAN_LABEL,
    "the namespace keeps its historical value so no saved plan needs migrating");
  // Both labels must be recognised, or a saved plan stops reading as generated.
  assert.ok(isGeneratedPlanLabel(GENERATED_PLAN_LABEL));
  assert.ok(isGeneratedPlanLabel(LEGACY_GENERATED_PLAN_LABEL));
  assert.ok(!isGeneratedPlanLabel("Four Years, Two Co-ops"));
  assert.ok(!isGeneratedPlanLabel(""));
  assert.ok(!isGeneratedPlanLabel(null));
});

test("provenance › a DEPARTMENT's variant label is still its own identity", () => {
  // The fix must not collapse published variants into one namespace: two different published
  // plans applied in turn are two different sets of cards, and de-duping them would lose one.
  const a = applyAs("Four Years, Two Co-ops in Spring/Summer First Half");
  const b = applyAs("Four Years, Two Co-ops in Summer Second Half/Fall", a);
  assert.equal(Object.keys(b.reservations).length,
    Object.keys(a.reservations).length * 2,
    "distinct published variants must NOT share provenance");
});
