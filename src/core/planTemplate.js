...
// Re-exported, not defined here: `applySamplePlan` needs them too and this file imports IT,
// so owning them here would make a cycle. Existing importers are unaffected.
export {
  GENERATED_PLAN_LABEL, LEGACY_GENERATED_PLAN_LABEL, GENERATED_ORIGIN_NS,
  isGeneratedPlanLabel,
} from "./planLabels.js";

/** Number words that appear as COUNTS in a published variant label. */
const COUNT_WORDS = {
...
/**
 * A variant label with its counting words written as digits.
 *
 * The picker sits in a ~190px column and these labels are long: measured over
 * the corpus, 181 distinct labels across 678 variants, the commonest is
 * "Four Years, Two Co-ops in Summer Second Half/Fall" at 49 characters and the
 * longest reaches 105. Numerals save a mean of 4.9 characters on the 140 of 181
 * that carry a number word, which is the difference between the co-op pattern
 * being readable and being ellipsized away — and the pattern is the whole
 * reason the picker exists.
 *
 * DISPLAY only. The label is the department's own scraped text and is the
 * variant's identity in `plan.json`, so it is never rewritten at the source.
 *
 * Whole words only, which is what keeps it safe: `\b` will not fire inside
 * "PlusOne", and the corpus contains no number word used as anything but a
 * count (measured: only one/two/three/four/five/six occur at all). Digits also
 * survive the runtime translator better than spelled-out numerals, which it
 * would otherwise have to translate per locale.
 */
export function shortVariantLabel(label) {
  return String(label ?? "").replace(
    /\b(one|two|three|four|five|six|seven|eight|nine|ten)\b/gi,
    (w) => COUNT_WORDS[w.toLowerCase()] ?? w,
  );
}
