...
// Pick the SMALLEST version the text fits into, then the strongest error
// correction that still fits that same version. So a short URL (a compact plan)
// lands on a tiny High-EC code — sparse, robust, roomy enough for a centre logo
// — while a big plan keeps its version small by dropping toward Low EC instead
// of ballooning. Returns { size, modules, ecl } (ecl = "H"|"Q"|"M"|"L"), or null
// if the text can't fit even a version-40 Low-EC code.
export function generateQr(text) {
  const low = encodeText(text, ECL_LOW); // Low always yields the smallest version
  if (!low) return null;
  for (const ecl of [ECL_HIGH, ECL_QUARTILE, ECL_MEDIUM, ECL_LOW]) {
    const enc = encodeText(text, ecl);
    if (enc && enc.version <= low.version)
      return { ...buildMatrix(enc.version, ecl, enc.dataCodewords), ecl: ecl.tag };
  }
  return { ...buildMatrix(low.version, ECL_LOW, low.dataCodewords), ecl: ECL_LOW.tag };
}
