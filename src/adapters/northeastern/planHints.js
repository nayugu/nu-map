/**
 * planHints.js — Northeastern's plan wording, read as evidence.
 *
 * Everything English- and CourseLeaf-shaped lives here, which is why it is not
 * in the solver: `requirementBinding.js` must run correctly with no hints at
 * all, just less often decisively.
 *
 * It sits in the northeastern adapter rather than in `scripts/` because the
 * RUNTIME needs it too. Binding is recomputed live so that an ambiguous card
 * offers its real candidates instead of the whole catalog, and that solve is
 * only as decisive as the evidence it is given. The spec helpers are injected
 * rather than imported, so this file knows nothing about core.
 *
 ...
 * a single sentence.
 *
 * Two kinds, and the solver treats them very differently:
 *
