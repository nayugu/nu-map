...
import META from "../core/dataMeta.json";

// Versioned by the data build, so a monthly rebuild is picked up instead of a
// long-open tab keeping the previous index forever. `lastUpdated` moves whenever
// the pipeline writes, which is exactly when this file changes.
const URL = `${import.meta.env.BASE_URL}northeastern/course-equivalences.json` +
            `?v=${encodeURIComponent(META.lastUpdated ?? "")}`;
...
async function load() {
  // Held for the session in production — the file is immutable between data
  // builds and the URL carries the version. In dev it is regenerated by hand,
  // so caching it in memory means an HMR update silently keeps the old index
  // and the panel shows results that no longer match the file on disk.
  if (cache && !import.meta.env.DEV) return cache;
  if (inflight) return inflight;                  // share, don't restart
  inflight = (async () => {
    try {
      const res = await fetch(URL, { cache: "no-cache" });   // revalidate, never serve stale
