 ...
 *
 * @property {CourseOffering} [offering]   - Optional per-term enrolment/schedule detail for
 *                                         COMPLETED terms, driving the availability gauges and the
 *                                         schedule/enrolment hover popovers. Undefined when the
 *                                         institution has no availability feed — the UI then simply
 *                                         omits those views. Term codes are raw registrar strings
 *                                         (decode via ICalendar).  See CourseOffering typedef.
 *
 * @typedef {Object} CourseOffering
 * @property {Record<string,number>} e   - Enrolled headcount per completed term code.
 * @property {Record<string,number>} c   - Capacity (max seats) per completed term code.
 *                                         open = max(0, c - e); fill% = e / c.
 * @property {Record<string,number>} s   - Section count per completed term code.
 *                                         open-per-section (the gauge colour) = open / s.
 * @property {number[]}    [dow]          - % of enrolment meeting each weekday [Mon..Fri], 0–100.
 * @property {[string,number][]} [pat]    - Meeting-pattern distribution [dayPattern, % of enrolment],
 *                                         most common first (top ~6; remainder is "other").
 *                                         Pattern letters: M T W R(Thu) F S(Sat) U(Sun), or "async".
 * @property {string[]}    [fmt]          - Instructional formats ever seen (union), e.g. ["Online"].
 * @property {string[]}    [cmp]          - Campuses ever seen (union), e.g. ["Boston"].
 * @property {boolean}     [lab]          - True if any section requires a linked lab/co-section.
 *
 * Curriculum attributes
 * @property {boolean}      isCps        - True if the course is offered through the College of Professional Studies.
 *                                         Derived from subject-colleges.json (generated by scrape-availability.js).
 *                                         False when college data has not yet been generated.
 *
